# -*- coding: utf-8 -*-
"""
Created on Mon Jul  3 11:07:40 2023
...
class PredRNNCell(nn.Module):
    def __init__(self, input_size, input_dim, hidden_dim_m, hidden_dim,kernel_size, bias):
        ...
        self.input_dim = input_dim #channel
        self.hidden_dim = hidden_dim  #lstm隐藏层单元，可以是第一层64,二128,三128
        self.hidden_dim_m = hidden_dim_m   #  hidden of M
        self.kernel_size = kernel_size
        self.padding = kernel_size[0] // 2, kernel_size[1] // 2  #(横竖网格大小不同）
        self.bias = bias
        #####################################################################################
        # 相应符号可对应参照论文
...
#####################################
    ...
    #
    #####################################
    def init_hidden(self, batch_size):#variable 是能反向传播的
