# -*- coding: utf-8 -*-
"""
Created on Mon Jul  3 11:15:36 2023
...
class PredRNN(nn.Module):
    def __init__(self, input_size, input_dim, hidden_dim, hidden_dim_m, kernel_size, num_layers,
                 batch_first=False, bias=True):#hidden_dim,  kernel_size可是是和num_layers一样len的list
        ...
        for i in range(0, self.num_layers):
            if i == 0:    # 0 时刻， 图片的输入即目前实际输入
                ...
                #Cell_list.appenda为堆叠层操作，存每层参数的尺寸/维度
        ...
        self.conv_last = nn.Conv2d(hidden_dim[num_layers-1], 1, kernel_size=(1,1), padding=0, bias=False)
    def forward(self, input_tensor, hidden_state=False, hidden_state_m=False):
        if self.batch_first is False:
            input_tensor = input_tensor.permute(1, 0, 2, 3, 4) #dim0,1换位置，目标格式（b,t,c,h,w）
        ...
        seq_len = input_tensor.size(1)   #第二个时间序列，3   步长
        cur_layer_input_1 = input_tensor #x方向上的输入  初始化输入数据
        #all_layer_out = []
        pre_h=[]
        next_frames=[]
        for t in range(seq_len):
            ...
            for layer_idx in range(self.num_layers): # 由于M在layer上传递,所以优先考虑layer上的传递
                if t == 0:  # 由于M在layer上传递，所以要区分t=0(此时m初始化)
                    ...
                    #c = cur_layer_input_c[layer_idx]  #原本的
            ...
            #alllayer_output = torch.cat(output_inner, dim=1) #把某个t时刻每个隐藏层的输出进行堆叠,以便于在解码层参照Convlstm使用1x1卷积得到输出 在1的位置堆叠
            #all_layer_out.append(alllayer_output)#记录每个t时刻,所有隐藏层输出的h,以便于在解码层参照Convlstm使用1x1卷积得到输出
            x_gen_h=self.conv_last(cur_layer_input[self.num_layers-1])
            #x_gen_c=self.conv_last(cur_layer_input_c[self.num_layers-1])
            pre_h.append(cur_layer_input[-1])
            next_frames.append(x_gen_h)#每个t的最后h
            #per_time_all_layer_stack_out=torch.stack(all_layer_out, dim=1)#记录每个t时刻,所有隐藏层输出的h,以便于在解码层参照Convlstm使用1x1卷积得到输出  又堆叠添加1维成为一个tensor，比alllayer_output多一维
        ...
        #return #torch.stack(layer_output_list, dim=1),\
            ...
            '''
        return output_inner_h_c,\
            torch.stack(last_state_list_m, dim=0),\
            torch.stack(pre_h, dim=1),\
            torch.stack(next_frames, dim=1)
